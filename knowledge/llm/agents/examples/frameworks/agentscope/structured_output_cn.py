...
def get_seer_model_cn(agents):
    names = tuple(a.name for a in agents)
    TargetEnum = enum.Enum("SeerTarget", {n: n for n in names})
    
    class SeerPick(BaseModel):
        target: TargetEnum = Field(description="必须在存活玩家中选择")
        check_reason: str
        priority_level: int = Field(ge=1, le=10)
        
        class Config:
            use_enum_values = True  # pydantic v1；v2 用 model_config
            model_config = True  # pydantic v1；v2 用 model_config
