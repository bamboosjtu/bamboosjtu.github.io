...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 5000):
        ...
        # 将 pe 注册为 buffer，这样它就不会被视为模型参数，但会随模型移动（例如 to(device)），[1, max_len, d_model]
...
class MultiHeadAttention(nn.Module):
    ...
    def split_heads(self, x):
        # 拆分多头，STEP2
        # 输入 x 的形状 (batch_size, seq_length, d_model)
    ...
    def combine_heads(self, x):
        # 拆分多头，STEP4
        # 输入 x 的形状 (batch_size, num_heads, seq_length, d_k)
        # 输出的形状 (batch_size, seq_length, d_model)
